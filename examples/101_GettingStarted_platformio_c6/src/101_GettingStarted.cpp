/*
  ...
  - Framework: arduino
  - USB CDC on boot: enabled through build flags
  - Serial monitor speed: 115200
  - This example targets CodeCell C6 and uses the same CodeCell library API as
  ...
  - myCodeCell.Init(LIGHT + MOTION_ROTATION);               // Light +
  orientation
  - myCodeCell.Init(LIGHT + MOTION_ROTATION + MOTION_STATE);// Light +
  orientation + state

  Run() Loop:
  - Example: myCodeCell.Run(10) returns true at 10 Hz (every 100 ms)
  - Read all enabled sensors at this rate (Range: 1Hz - 100Hz)
  - The same call updates power status, reads enabled sensors, and drives the
  status LED.
  - Low battery behavior: below ~3.3 V the LED blinks red 10×, then the unit
  sleeps until USB is connected.
  ...
  - Motion_RotationVectorRead(float &vec_r, float &vec_i, float &vec_j, float
  &vec_k)
  ...
  GPIO Control:
  - myCodeCell.pinWrite(uint8_t pin, bool value);              // Write HIGH/LOW
  to pin
  - myCodeCell.pinRead(uint8_t pin);                           // Read digital
  pin state
  - myCodeCell.pinPWM(uint8_t pin, uint16_t freq, uint8_t dc); // Generate PWM
  (frequency, duty cycle)
  - myCodeCell.pinADC(uint8_t pin);                            // Read analog
  value from pin
  ...
  MicroLink App:
  - Download the CodeCell MicroLink library to connect with the companion
  smartphone App
...
void setup() {
  Serial.begin(115200); // Start USB serial at 115200 baud (enable
                        // USB_CDC_On_Boot for Serial)
