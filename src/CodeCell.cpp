...
void CodeCell::Motion_Read() {
  ...
  const uint32_t REPORT_RECOVERY_TIMEOUT = 500;
  const uint32_t SOFT_RESET_TIMEOUT = 1000;
  const uint32_t SOFT_RESET_RETRY_DELAY = 250;

  // 0 = normal
  // 1 = reports restored, waiting for sensor data
  // 2 = soft reset sent, waiting for reset notification
  // 3 = waiting before another soft-reset attempt
  static uint8_t recovery_state = 0;
  static uint32_t recovery_timer = 0;
  static uint32_t reset_detected_time = 0;
  ...
  // A reset notification can be generated by an unexpected BNO085
  if (Motion.wasReset()) {
    uint32_t reset_event_time = millis();

    // Start timing only when beginning a new recovery.
    // Do not restart the timer after a software-generated reset.
    if (recovery_state == 0) {
      reset_detected_time = reset_event_time;
    }

#if CODECELL_DEBUG
    Serial.print(">> [");
    Serial.print(reset_event_time);
    Serial.println(" ms] BNO085 reset detected");
#endif

    delay(50);
    Motion_EnableReports(false);

    recovery_state = 1;
    recovery_timer = millis();
#if CODECELL_DEBUG
    Serial.println(">> Waiting for BNO085 data");
#endif
    return;
  }

  // Receiving any motion event confirms that recovery succeeded
  if (received_event) {
    if (recovery_state != 0) {
#if CODECELL_DEBUG
      Serial.print(">> [");
      Serial.print(millis());
      Serial.print(" ms] BNO085 recovered after ");
      Serial.print(millis() - reset_detected_time);
      Serial.println(" ms");
#endif
    }
  ...
  if ((recovery_state == 1) && (millis() - recovery_timer >= REPORT_RECOVERY_TIMEOUT)) {

#if CODECELL_DEBUG
    Serial.print(">> [");
    Serial.print(millis());
    Serial.println(" ms] No BNO085 data: sending soft reset");
#endif
    ...
    if (Motion.softReset()) {
      ...
      Serial.print(">> [");
      Serial.print(millis());
      Serial.println(" ms] BNO085 soft reset command failed");
#endif
  ...
  // The soft-reset command succeeded, but no SH2_RESET notification arrived.
  if ((recovery_state == 2) && (millis() - recovery_timer >= SOFT_RESET_TIMEOUT)) {
#if CODECELL_DEBUG
    Serial.print(">> [");
    Serial.print(millis());
    Serial.println(" ms] BNO085 soft reset timed out: waiting before retry");
#endif
    recovery_state = 3;
    recovery_timer = millis();
    return;
  ...
  if ((recovery_state == 3) && (millis() - recovery_timer >= SOFT_RESET_RETRY_DELAY)) {

    // Enter state 1 with its timeout already elapsed,
    // causing another soft-reset attempt on the next call.
    recovery_state = 1;
    recovery_timer = millis() - REPORT_RECOVERY_TIMEOUT;
    return;
  }
}
...
void CodeCell::Motion_EnableReports(bool print_status) {

  if ((_msense & MOTION_ACCELEROMETER) == MOTION_ACCELEROMETER) {
    if (Motion.enableAccelerometer() == true) {
      if (!_wakeup_flag&& print_status) {
